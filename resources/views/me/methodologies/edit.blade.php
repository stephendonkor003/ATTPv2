...
<div class="nxl-container">
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Update methodology details.</p>
        </div>
    </div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body">
            @php
                ...
                $initialSurveyQuestions = old(
                    'survey_questions_json',
                    json_encode((array) data_get($surveyMeta, 'questions', []))
                );
                $surveyEnabled = old('survey_public_enabled', data_get($surveyMeta, 'enabled', true));
                $surveyTitle = old(
                    'survey_title',
                    data_get($surveyMeta, 'title', ($methodology->name . ' Public Survey'))
                );
                $surveyIntro = old('survey_intro', data_get($surveyMeta, 'intro', ''));
            @endphp
            ...
            <form method="POST" action="{{ route('budget.me-configuration.methodologies.update', $methodology) }}" class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" id="methodologyNameInput" class="form-control"
                        ...
                        placeholder="e.g., Survey, Field Observation, Desk Review" required>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-check form-switch mt-2">
                        <input class="form-check-input" type="checkbox" name="is_active" value="1" {{ old('is_active', $methodology->is_active) ? 'checked' : '' }}>
                ...
                <div class="col-12 d-none" id="surveyBuilderPanel">
                    <div class="survey-builder-panel">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                            <div>
                                ...
                                <p class="survey-help mb-0">
                                    Configure the public survey used by indicators that choose this methodology.
                                    Generated public links can be copied and shared with the general public.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" name="survey_title" id="surveyTitleInput" class="form-control"
                                    value="{{ $surveyTitle }}" placeholder="Public survey title">
                            </div>
                            <div class="col-12">
                                <label class="form-label">Intro / Instructions</label>
                                <textarea name="survey_intro" id="surveyIntroInput" rows="2" class="form-control"
                                    placeholder="Write simple instructions for external respondents.">{{ $surveyIntro }}</textarea>
                            </div>
                        </div>
                        ...
                        <input type="hidden" name="survey_questions_json" id="surveyQuestionsJson"
                            value="{{ $initialSurveyQuestions }}">
                        <div id="surveyQuestionsContainer" class="d-grid gap-2"></div>
                        <small class="text-muted d-block mt-2">Supported field types: text, textarea, number, email,
                            date, select, radio, checkbox.</small>
                    </div>
                </div>
