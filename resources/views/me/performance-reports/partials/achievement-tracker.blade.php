@php
    ...
    $reportingScope = $report->reporting_scope ?? [];
@endphp
            ...
            @if ($achievementEditable)
                <form method="POST" action="{{ route('budget.me.performance-reports.achievements.store', [$report, $result]) }}" class="achievement-form">
                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-md-3"><label class="form-label">Geographic scope</label><select name="geographic_scope" class="form-select" required>@foreach($achievementTaxonomy['geographic_scopes'] as $value=>$label)<option value="{{ $value }}" @selected(($reportingScope['geographic_scope'] ?? null)===$value)>{{ $label }}</option>@endforeach</select></div>
                        <div class="col-md-3"><label class="form-label">Country</label><input name="country" class="form-control" list="attp-country-options" value="{{ $reportingScope['country'] ?? $report->thinkTank?->country }}" maxlength="120"></div>
                        <div class="col-md-3"><label class="form-label">REC</label><select name="rec" class="form-select"><option value="">Not applicable</option>@foreach($achievementTaxonomy['recs'] as $value=>$label)<option value="{{ $value }}" @selected(($reportingScope['rec'] ?? null)===$value)>{{ $label }}</option>@endforeach</select></div>
                        ...
                        <div class="col-12">
                            ...
                            <div class="achievement-theme-grid">@foreach($achievementTaxonomy['priority_themes'] as $value=>$label)<label class="achievement-theme-option"><input type="checkbox" name="priority_themes[]" value="{{ $value }}" @checked(($reportingScope['priority_theme'] ?? null)===$value)> {{ $label }}</label>@endforeach</div>
                        </div>
                        ...
                        <div class="achievement-subsection">
                            ...
                            @if($achievementEditable)
                                <form method="POST" action="{{ route('budget.me.performance-reports.achievements.breakdowns.store',[$report,$achievement]) }}" class="achievement-breakdown-form">
                                    @csrf
                                    <div><label class="form-label">Scope{{ $requiredCodes->contains('geographic_scope')?' *':'' }}</label><select name="geographic_scope" class="form-select"><option value="">Not reported</option>@foreach($achievementTaxonomy['geographic_scopes'] as $value=>$label)<option value="{{ $value }}" @selected(($reportingScope['geographic_scope'] ?? $achievement->geographic_scope)===$value)>{{ $label }}</option>@endforeach</select></div>
                                    <div><label class="form-label">Country{{ $requiredCodes->contains('country')?' *':'' }}</label><input name="country" class="form-control" list="attp-country-options" value="{{ $reportingScope['country'] ?? $achievement->country }}"></div>
                                    <div><label class="form-label">REC{{ $requiredCodes->contains('rec')?' *':'' }}</label><select name="rec" class="form-select"><option value="">Not applicable</option>@foreach($achievementTaxonomy['recs'] as $value=>$label)<option value="{{ $value }}" @selected(($reportingScope['rec'] ?? $achievement->rec)===$value)>{{ $label }}</option>@endforeach</select></div>
                                    ...
                                    <div><label class="form-label">Theme{{ $requiredCodes->contains('priority_theme')?' *':'' }}</label><select name="priority_theme" class="form-select"><option value="">Not reported</option>@foreach($achievementTaxonomy['priority_themes'] as $value=>$label)<option value="{{ $value }}" @selected(($reportingScope['priority_theme'] ?? null)===$value)>{{ $label }}</option>@endforeach</select></div>
                                    <div><label class="form-label">Gender{{ $requiredCodes->contains('gender')?' *':'' }}</label><select name="gender" class="form-select"><option value="">Not reported</option>@foreach($achievementTaxonomy['genders'] as $value=>$label)<option value="{{ $value }}" @selected(($reportingScope['gender'] ?? null)===$value)>{{ $label }}</option>@endforeach</select></div>
                                    <div><label class="form-label">Age{{ $requiredCodes->contains('age_group')?' *':'' }}</label><select name="age_group" class="form-select"><option value="">Not reported</option>@foreach($achievementTaxonomy['age_groups'] as $value=>$label)<option value="{{ $value }}" @selected(($reportingScope['age_group'] ?? null)===$value)>{{ $label }}</option>@endforeach</select></div>
                                    <div><label class="form-label">Stakeholder category{{ $requiredCodes->contains('stakeholder_category')?' *':'' }}</label><select name="stakeholder_category" class="form-select"><option value="">Not reported</option>@foreach($achievementTaxonomy['stakeholder_categories'] as $value=>$label)<option value="{{ $value }}" @selected(($reportingScope['stakeholder_category'] ?? null)===$value)>{{ $label }}</option>@endforeach</select></div>
