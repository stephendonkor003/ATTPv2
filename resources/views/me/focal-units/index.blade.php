...
<div class="container-fluid">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3"><div class="card border-0 shadow-sm"><div class="card-body"><div class="text-muted small">Accounts linked</div><div class="fs-3 fw-bold {{ $contacts->whereNotNull('user_id')->count()===$contacts->count()?'text-success':'text-warning' }}">{{ $contacts->whereNotNull('user_id')->count() }} / {{ $contacts->count() }}</div></div></div></div>
    </div>
    ...
    @foreach($contacts->groupBy('consortium_name') as $consortium => $consortiumContacts)
        ...
        @foreach($consortiumContacts as $contact)
            ...
            <tr><td><div class="fw-semibold">{{ $contact->think_tank_label }}</div><div class="text-muted small">{{ $contact->thinkTank?->name ?: 'Not mapped to platform organization' }}</div></td><td><div class="fw-semibold">{{ $contact->focal_person_name }} @if($contact->is_primary)<span class="badge bg-success">Primary</span>@endif</div><a href="mailto:{{ $contact->email }}" class="small">{{ $contact->email }}</a>@if($contact->notes)<div class="small text-muted">{{ $contact->notes }}</div>@endif</td><td>@if($match)<div>{{ $match->name }}</div><div class="small text-muted">{{ $match->email }}</div>@else<span class="badge bg-light text-danger border">No matching account</span><div class="small text-muted mt-1">Create an account with this exact email, then return here.</div>@endif</td><td>@if($contact->user && $contact->user->think_tank_member_id===$contact->think_tank_member_id && $contact->user->think_tank_access_level===\App\Models\User::THINK_TANK_ACCESS_ME)<span class="badge bg-success">Ready to report</span>@elseif($match && $contact->think_tank_member_id)<span class="badge bg-warning text-dark">Account found; link required</span>@else<span class="badge bg-light text-danger border">Setup incomplete</span>@endif</td><td class="text-end"><div class="d-flex justify-content-end gap-2">
                @can('me.configuration.manage')
