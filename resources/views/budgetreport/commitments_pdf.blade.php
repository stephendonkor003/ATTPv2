...
<body>
    <div class="header">
        <div class="title">{{ $program->name ?? 'Commitment Report' }}</div>
    ...
    <div class="section">
        ...
        <br>
        <strong>Total Allocated:</strong> {{ $currency }} {{ number_format($totals['allocated'] ?? 0, 2) }}
        &nbsp;&nbsp;
        <strong>Total Committed:</strong> {{ $currency }} {{ number_format($totals['committed'] ?? 0, 2) }}
    </div>

    <div class="section">
        <h3 style="margin: 0 0 8px 0; color: #0f172a;">Section 1: Commitment Balance Sheet</h3>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($rows as $projectRow)
                    <tr class="project-row">
                        ...
                        <td class="right">{{ number_format($projectRow['allocated'], 2) }}</td>
                        ...
                        @foreach ($filters['year_range'] as $year)
                            <td class="right">{{ number_format($projectRow['yearly']['allocated'][$year] ?? 0, 2) }}</td>
                    ...
                    @foreach ($projectRow['activities'] as $activityRow)
                        <tr class="activity-row">
                            ...
                            <td class="right">{{ number_format($activityRow['allocated'], 2) }}</td>
                            ...
                            @foreach ($filters['year_range'] as $year)
                                <td class="right">{{ number_format($activityRow['yearly']['allocated'][$year] ?? 0, 2) }}</td>
                        ...
                        @foreach ($activityRow['subActivities'] as $subRow)
                            <tr class="sub-row">
                                ...
                                <td class="right">{{ number_format($subRow['allocated'], 2) }}</td>
                                ...
                                @foreach ($filters['year_range'] as $year)
                                    <td class="right">{{ number_format($subRow['yearly']['allocated'][$year] ?? 0, 2) }}</td>
    ...
    <div class="section">
        ...
        @else
            <table>
                <thead>
                    <tr>
                        <th style="width: 35%;">Line Chart (Commitments Over Time)</th>
                        <th style="width: 35%;">Bar Chart (Allocations vs Commitments)</th>
                        ...
                        <td>
                            @if (!empty($chartData['line']['labels']))
                                @foreach ($chartData['line']['labels'] as $index => $label)
                                    <div>{{ $label }}: {{ number_format($chartData['line']['data'][$index] ?? 0, 2) }}</div>
                                @endforeach
                            @else
                                N/A
                            @endif
                        ...
                        <td>
                            @if (!empty($chartData['bar']['labels']))
                                @foreach ($chartData['bar']['labels'] as $index => $label)
                                    <div>
                                        {{ $label }}:
                                        Alloc {{ number_format($chartData['bar']['allocations'][$index] ?? 0, 2) }},
                        ...
                        <td>
                            @if (!empty($chartData['bubble']))
                                @foreach ($chartData['bubble'] as $bubble)
                                    <div>{{ $bubble['label'] ?? 'Sub-Activity' }}: Alloc {{ number_format($bubble['x'] ?? 0, 2) }}, Commit {{ number_format($bubble['y'] ?? 0, 2) }}</div>
                                @endforeach
                            @else
                                N/A
                            @endif
    ...
    <div class="section">
        <h3 style="margin: 0 0 8px 0; color: #0f172a;">Section 3: AI Summary</h3>
        <ul>
