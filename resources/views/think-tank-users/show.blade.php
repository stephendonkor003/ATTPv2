            ...
            <aside class="ttud-side">
                <section class="ttud-panel">
                    ...
                    <div class="ttud-security-body">
                        ...
                        <div class="ttud-security-item"><span><i class="feather-mail"></i></span><div><strong>Secure reset link</strong><small>The system sends a single-use link first. Only after Microsoft accepts delivery are the current password, MFA challenge and active sessions revoked.</small></div></div>
                        <form class="ttud-reset" method="POST" action="{{ route('system.think-tank-users.reset-password', $user) }}" onsubmit="return confirm('Send this user a secure, single-use password reset link and revoke existing access after delivery is accepted?')">@csrf<button class="ttud-btn warn" type="submit"><i class="feather-refresh-cw"></i> Send secure reset link</button></form>
