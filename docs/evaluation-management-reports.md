...
Grouped notifications use the application's default mailer. Production must set `MAIL_MAILER=graph` so these messages use the same Microsoft Graph transport as every other application email. Notification delivery runs after the batch commits, and any failure is recorded against every affected rework request.
